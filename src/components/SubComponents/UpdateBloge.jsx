...
const UpdateBloge = ({textareaRef,handleinput,settextchange,textchange,getblogs,setshowupdate}) => {
    ...
    const oninputchange = (e)=>{
        const val  = e.target.name === "file"? e.target.files[0]: e.target.val
        setblogdata(prev =>({...prev,[e.target.name]:val}))

    }
    const onformSubmit =async (e)=>{
            e.preventDefault()
            const responce = await axios.put(`http://localhost:5001/blog/update/${id}`,{blogdata},{headers:{"Content-Type":"multipart/form-data","Authorization":localStorage.getItem("token")}})
